...
- 🤖 **AI Study Layer, Powered by Gemini** – Get step-by-step problem solutions, chat with the AI about the exact PDF you're reading, generate revision notes, and surface high-probability topics from historical question trends, all while reading the actual document content instead of just its title.
...
- 🧹 **Self-Cleaning Trash** – Deleted and replaced files are automatically purged, including from GitHub, 30 days after deletion, with no manual cleanup needed.
...
### Part 1: Get the site running and get yourself admin access

1. Requires Node.js 18 or later.

2. Create a new [Supabase](https://supabase.com) project, then open its SQL Editor and run the entire contents of `supabase/schema.sql` in one go. This creates every table, function, trigger, and access rule the app needs. Do this before anything else, since every other step depends on it.

3. In Supabase, go to Authentication → Providers and enable **Google** as a sign-in provider. This is what the admin dashboard's "Sign in with Google" button uses. Follow Supabase's own guide for creating the Google OAuth credentials it asks for.
...
5. Set up environment variables for the frontend. Create a `.env.local` in the project root:
...
`VITE_JSDELIVR_OWNER`, `VITE_JSDELIVR_REPO` and `VITE_JSDELIVR_BRANCH` point at the GitHub repo that will hold your PDFs, set up in Part 2 below. It's fine to fill these in with placeholder values for now and come back to them later.
...
7. In Supabase's SQL Editor, run this once with your own email in place of the placeholder. Do this before you try to sign in anywhere:

```sql
insert into admins (email, role) values ('you@example.com', 'owner');
```

8. Sign in with that same Google account. Login is restricted to admins only, and there is no self-service admin signup in the dashboard, so registering your email in the `admins` table first is what allows the sign-in to succeed. You'll land with full dashboard access.

At this point the site runs, you can browse it, and you have an admin account. File uploads and publishing won't work yet; that's Part 2.

### Part 2: Set up file publishing

The dashboard's publish/upload features (approving submissions, adding resources, replacing files) need a GitHub token and a few other secrets to actually reach GitHub, and the automated 30-day cleanup needs its own secret too. Skip this if you only need the site running for now.

1. Create a second, separate public GitHub repository to hold the actual PDF files (referenced below as `civil-pyq-pdf`). This is not the same repo as this codebase. Update your `.env.local` from Part 1 with its real owner/repo/branch values.

2. Install the Supabase CLI and connect it to your project:
...
`supabase login` opens a browser window to authenticate the CLI with your Supabase account. Your project ref is the short ID in your project's dashboard URL (`supabase.com/dashboard/project/<this-part>`), also shown under Project Settings → General.

3. Create a fine-grained GitHub Personal Access Token scoped only to the PDF repo from step 1, with **Contents: Read and write** permission. Then set these as Supabase Edge Function secrets:
...
`GITHUB_OWNER`, `GITHUB_REPO` and `GITHUB_BRANCH` must exactly match the `VITE_JSDELIVR_*` values from Part 1, otherwise the URLs stored in the database won't resolve.
...
5. On the GitHub repo that hosts *this* codebase (not the PDF repo), add two Actions secrets under Settings → Secrets and variables → Actions: `SUPABASE_URL` (your project URL) and `CRON_SECRET` (the exact same value from step 3). This is what lets the daily scheduled workflow authenticate to `purge-expired`, covered under "Scheduled Cleanup" below.

## How It Works

CivilPYQ splits into two simple pieces: **Supabase** holds the structured data (subjects, resource listings, submissions, admin accounts), while the actual PDF files live in a separate, public **GitHub** repository, dedicated solely to storing the PDFs and distinct from this frontend repo, and are served worldwide through the **jsDelivr CDN**. Nothing large ever passes through Supabase; it only ever stores a link.
...
- **Scheduled** – requires an end time. Once that time passes, the site is live again automatically and instantly, for every visitor, computed fresh on every page load rather than flipped by a background job.
- **Indefinite** – no end time. Stays active until an admin explicitly ends it.

While a scheduled window is active, its end time and the visitor-facing message can be edited freely. Extending or shortening it just works, since it's a live value being read, not a snapshot. Once a window has ended (or hasn't been started), starting a new one always requires a fresh choice, so there's no way for an old end time to silently linger and unexpectedly reactivate later.
...
- The function checks that the caller is a signed-in admin with edit rights, and that the file is under 19MB, since jsDelivr hard-rejects anything over 20MB regardless of what GitHub itself would accept, so 19MB is enforced everywhere (client-side, both relevant edge functions, and the public submissions storage bucket) to leave headroom.
...
Every admin who should be able to publish files needs one of `can_review_submissions`, `can_edit_resources`, or `can_manage_contributors` set to `true` in the `admins` table (or `role = 'owner'`). The full setup for this pipeline (GitHub token, edge function secrets, deploying the functions) is covered in Installation above.
